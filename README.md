...
超售是线程安全问题的一种，使用锁是一个很容易想到的方案。根据不同的策略，锁可以分为乐观锁和悲观锁。
...
### 一人一单
对于某种优惠券，为限制同一用户只能购买一张，我们使用同步代码块 (synchronized statements) 来避免并发情况下的「线程干扰」问题。
我们可以将同步 (synchronization) 也看作一种锁。

但测试后发现使用同步似乎还是不能完全避免这个问题，存在同一用户能下两单的情况。

另外，在应用集群化部署时，应用分别运行在不同的**进程**中，同步就不再起作用了，因为同步只能保证同一个进程内的不同线程串行地执行同步代码块。
此时，需要想新的办法来保证线程安全，这个办法就是 —— **分布式锁**。

### 基于 Redis 实现分布式锁
针对上面提到的，在应用集群化部署环境下不再能保证不同进程中线程的同步，从而无法确保同一用户只能下一单的问题，
我们需要的是一个不同的进程中的所有线程都可见且能申请的互斥锁，也就是所谓的**分布式锁 (Distributed Locking)**。

分布式锁有不同的实现方式，使用 MySQL、Redis 和 ZooKeeper 都能实现该功能，但各有优缺点。
我们这个项目的目的是学习 Redis 的使用，因此我们这里使用 Redis 来实现。对于分布式锁的不同实现，评价指标主要是：
* 互斥的实现方式
* 可用性
* 性能
* 安全性
